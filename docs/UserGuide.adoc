...
[caption=]
.Figure 2: Simple diagram of data storage in Volant
...
.Figure 3: The different components of a page in Volant
[caption=]
...
[caption=]
.Table 3: Types of Pages in Volant
[cols="1, 3", options="header"]
|===
| Page                  | Details
|`HOME` page            a|image::user-guide/home-page.png[width="300"]
Volant's Home page, featuring your entire list of trips in Volant, sorted in upcoming and past `TRIP` s.
|`TRIP` page            a|image::user-guide/trip-page.png[width="300"]
A page featuring the details of your specific trip, including the `TRIP` 's `ITINERARY` and `JOURNAL`.
|`JOURNAL` page         a|image::user-guide/journal-page.png[width="300"]
A page displaying the `JOURNAL` of your specific trip.
|`ITINERARY` page       a|image::user-guide/itinerary-page.png[width="300"]
A page displaying the `ITINERARY` of your specific trip.
|===
...
[caption=]
.Figure 5: After you have used the `find` command on the `HOME` page, only the results of the command will be displayed.
image::user-guide/refresh-command-before.png[width="300"]

[caption=]
.Figure 6: After using the `rf` command, the `HOME` page returns to its original state and lists all trips.
...
[caption=]
.Figure 7: You are on the `ITINERARY` page of a specific `TRIP`
image::user-guide/back-command-before.png[width="300"]

[caption=]
.Figure 8: After using the `back` command, you will be moved to the `TRIP` page of the specific `TRIP`
...
====
[caption=]
.Table 4: Outcomes when using the `back` command on certain pages
...
[caption=]
.Figure 9: You are on the `ITINERARY` page of a specific `TRIP`
image::user-guide/back-command-before.png[width="300"]



[caption=]
.Figure 10: After using the `home` command, you will be moved to the `HOME` page
image::user-guide/home-command-after.png[width="300"]

=== Commands for the Home Page

The `HOME` page consists of a list of all your upcoming and past trips.

.Figure 11: The different components displayed in the Home page.
...
[caption=]
.Figure 12: You are on the `HOME` page and you want to navigate to the `TRIP` page of the 3rd `TRIP`, _Winter Break_.
image::user-guide/home-goto-before.png[width="300"]

[caption=]
.Figure 13: After using the `goto` command, you will be navigated to the 3rd `TRIP`, _Winter Break_.
...
[caption=]
.Figure 14: Home Page before adding your trip.
...
[caption=]
.Figure 15: Home Page after trip has been added.
...
[caption=]
.Figure 18: Home page before editing.
...
[caption=]
.Figure 19: Home page after editing.
.2) After you use the `edit` command, the `TRIP_NAME` and `LOCATION` of your trip will be changed accordingly.
...
[caption=]
.Figure 20: Home page before calling `find` command.
...
[caption=]
.Figure 21: Home page after calling `find` command.
...
[caption=]
.Figure 22: GUI displaying Trip page before `goto` command.
.1) You are on the `TRIP` page of the trip "Family Trip"
image::user-guide/trip-page.png[width="300"]

[caption=]
.Figure 23: GUI displaying Itinerary page after `goto` command.
...
.Figure 24: The different components displayed in an Itinerary page.
...
[caption=]
.Figure 27: Itinerary page before `delete` command is called.
.You want to delete the 4th activity in your itinerary for the trip.
image::user-guide/itinerary-delete-before.png[width="600"]

[caption=]
.Figure 28: Itinerary page after `delete` command is called.
.After using the `delete` command, the trip will be removed from the itinerary's activity list.
...
[caption=]
.Figure 29: Itinerary page before `edit` command is called.
.1) You want to edit the `LOCATION` and `TIME` of the second activity in your activity list.
image::user-guide/itinerary-edit-before.png[width="600"]

[caption=]
.Figure 30: Itinerary page after `edit` command is called.
.2) After using the `edit` command, the `LOCATION` and `TIME` of the activity will be changed accordingly.
