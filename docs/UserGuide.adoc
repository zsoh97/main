...
2) After using the `rf` command, the `HOME` page will return to its original state and will lists all trips.
...
.Outcomes when using the `back` command on certain pages
