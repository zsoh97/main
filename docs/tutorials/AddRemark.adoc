...
This command allows users of the AddressBook application to add optional remarks to people in their location book and edit it if required.
...
/**
 * Changes the remark of an existing trip in the location book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the trip identified "
            + "by the index number used in the last trip listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the trip in the filtered trip list to edit the remark
     * @param remark of the trip to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.location.logic.parser` package.
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the trip's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a trip.

=== Add a new `Remark` class

Create a new `Remark` in `trip`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each trip.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.location.ui.TripCard`].
...
Not doing so will cause AddressBook to default to an empty location book!
...
.PersonCard.java
----
public PersonCard(Person trip, int displayedIndex) {
    //...
    remark.setText(trip.getRemark().value);
}
----
