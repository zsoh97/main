...
It is technical, and explains the inner workings of Volant and our chosen implementation for certain features.
...
==== Four Pages - `HOME`, `TRIP`, `ITINERARY`, `JOURNAL`
...
====
For example, if a user is switching from the `HOME` view to the `TRIP` view through the command, `goto`, to
access a specific trip's page.

. Reassign the `Logic` component from `HomePageLogicManager` to a new `TripPageLogicManager`.
. Reassign the `Model` component from `HomePageModelManager` to a new `TripPageModelManager`.
. Reassign the value of `StackPane` `mainPanel` from `HomePage` to `TripPage`.
