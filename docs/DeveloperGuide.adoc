...
By: `AY1920S2-CS2103T-F09-4`      Since: `Jan 2020`      Licence: `???`
...
* <<Design-Model,*`Model`*>>: Holds the data of the App in-memory dependong in the view the user is on.
...
There are 4 views in this App: `HOME`, `TRIP`, `ITINERARY`, `JOURNAL`.

For the `Logic` and `Model` components, each view has their own `{Component Name} Manager` classes.

e.g. `HomeLogicManager` etc. to facilitate different operations and commands in different views in the App.


.Class Diagram of the Home Logic Component
image::LogicDiagram.png[]

[discrete]
==== How the architecture components interact with each other

The _Sequence Diagram_ below shows how the components interact with each other for the scenario where the user issues the command `back` when in a `TRIP` view.
...
.  `Logic` uses the `XYZInputParser` class of the current feature to parse the user command.
.  This results in a `Command` object which is executed by the `XYZLogicManager` of the current feature.
...
*Value proposition*: convenient travel assistant for solo travelers who are always on their feet
...
[discrete]
=== UC08 - Deleting a Contact in the Contact List of aTrip
...
. Deleting an activity in the activity list
...
.. Other incorrect delete commands to try: `delete`, `delete x` (where x is larger than the itinerary size) _{give more}_ +
   Expected: Similar to previous.
...
. Deleting an entry in the entry list
...
.. Other incorrect delete commands to try: `delete`, `delete x` (where x is larger than the journal size) _{give more}_ +
   Expected: Similar to previous.
