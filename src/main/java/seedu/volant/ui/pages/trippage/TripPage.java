...
public class TripPage extends UiPart<Region> {
    private static final String FXML = "TripPage.fxml";
    ...
    public TripPage(Trip trip) {
        ...
        tripFeaturesListView.setItems(trip.getTripFeatureList().getReadOnly());
        ...
        protected void updateItem(TripFeature tripFeature, boolean empty) {
            ...
            if (empty || tripFeature == null) {
                ...
                if (tripFeature instanceof Journal) {
                    setGraphic(new JournalCard(tripFeature).getRoot());
                } else {
                    setGraphic(new ItineraryCard(tripFeature).getRoot());
                }
            }
        }
    }
